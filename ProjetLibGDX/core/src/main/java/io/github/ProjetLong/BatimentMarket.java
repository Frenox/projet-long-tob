...
public class BatimentMarket implements Batiment{
    ...
    public void logic(VilleScreen screen) {
        ...
        if (this.isOpened && this.isSelling == false){
            //changer de page
            if (mouse.x >= 216 && mouse.x <= 265 && mouse.y >= 56 && mouse.y <= 67 && page < (((poissons.size() - 1)/7))) {
                page++;
            }
    ...
    private void affichageInterface(VilleScreen screen) {
        ...
        for (int i = (page * 7); i != (7 * (page + 1)); i++) {
            if (i < len) {
                ...
                screen.jeu.HebertBold.draw(screen.jeu.batch, poissons.get(i).getNom(), 146f, 210.5f - ((i % 7) * 19) - 15);
            }
        }
        screen.jeu.HebertBold.draw(screen.jeu.batch, Integer.toString(page + 1), 197.5f - (5*(page/9)), 43f + 20);
    ...
    private void affichageVente(VilleScreen screen) {
        ...
        //screen.jeu.HebertBold.draw(screen.jeu.batch, "pour " + this.poissons.get(selectedfish).getValeur() + " coins ?", 140, 220);
